...
@views function main(; nx=128, ny=128, backend=:auto, verbose=true)
    ...
    ∇Vs     = CRArray(zeros(nx - 0, ny - 0))
    RVx     = CRArray(zeros(nx - 1, ny - 0))
    RVy     = CRArray(zeros(nx - 0, ny - 1))
    Rτxx    = CRArray(zeros(nx, ny))
    Rτyy    = CRArray(zeros(nx, ny))
    Rτxy    = CRArray(zeros(nx + 1, ny + 1))
    # initial conditions
    Pt      = CRArray(P0 .* ones(nx, ny))
    ηs_v    = ηs0 .* ones(nx + 1, ny + 1)
    ηs_v[(xv.^2 .+ (yv').^2).^(0.5) .< radius] .= ηs_inc
    ηs_v    = CRArray(ηs_v)
    ηs      = av(ηs_v) |> CRArray
    iter    = CRNumber(0)
    err     = CRNumber(10tol)
    ...
    plt = (heatmap!(axs[1], xc,  yc, to_array(Pt); colorrange=(-3,3), colormap=(CairoMakie.Reverse(:matter), 1)),
           heatmap!(axs[2], xv,  yce,to_array(Vxs); colormap=:turbo),
           heatmap!(axs[3], xce, yv, to_array(Vys); colormap=:turbo))
    ...
    # pt loop
    @views function step(Pt, Vxs, Vys, τxx, τyy, τxy, ∇Vs, RVx, RVy, Rτxx, Rτyy, Rτxy, ηs, ηs_v, dτ_r, νdτ, dτ_Pr, dx, dy, nout, iter, err, err_log)
        iter += 1
    ...
    function solve(Pt, Vxs, Vys, τxx, τyy, τxy, ∇Vs, RVx, RVy, Rτxx, Rτyy, Rτxy, ηs, ηs_v, dτ_r, νdτ, dτ_Pr, dx, dy, tol, maxiter, nout, iter, err, err_log)
        @trace while (iter < maxiter) & (err >= tol)  # @trace is a no-op without Reactant
            iter, err = step(Pt, Vxs, Vys, τxx, τyy, τxy, ∇Vs, RVx, RVy, Rτxx, Rτyy, Rτxy, ηs, ηs_v, dτ_r, νdτ, dτ_Pr, dx, dy, nout, iter, err, err_log)
        end
        return iter, err
    end
    ...
    # visualisation
    plt[1][3] = to_array(Pt) .- P0
    plt[2][3] = to_array(Vxs)
    plt[3][3] = to_array(Vys)
    display(fig)

    return
end

main(nx=512, ny=512, backend=:cpu, verbose=false)
